...
public class ControlPanel extends VBox {
  ...
  private final Button btnCarTreedButton;
  private final Button btnTruckTreedButton;
  ...
  private boolean CarBt;
  private boolean TruckBt;
...
public void setSimulationService(SimulationService service) {
  ...
  public TextField getTxtCarLifetime() {
    return txtCarLifetime;
  }

  public TextField getTxtTruckLifetime() {
    return txtTruckLifetime;
  }
  ...
  public ControlPanel() {
    ...
    tbStop.setStyle(
        "-fx-background-color: #db1515; -fx-text-fill: white; -fx-font-weight: bold;"
    ...
    btnCarTreedButton = new Button("машины");
    btnTruckTreedButton = new Button("грузовики");
    btnStop.setStyle(
        "-fx-background-color: #f03131; -fx-text-fill: white; -fx-font-weight: bold;"
            + " -fx-background-radius: 5;");
    btnStop.setDisable(true);
    //кнопки для потоков 
    btnCarTreedButton.setOnAction(e -> {
    if (simulationService.recipientCarPaused()) {
        // поток работает → ставим на паузу
        simulationService.resumeCarAI();
    } else {
        // поток на паузе → запускаем
        simulationService.pauseCarAI();
    }
});
    
    btnTruckTreedButton.setOnAction(
        e -> {
          if (simulationService.recipientTruckPaused()) {
        // поток работает → ставим на паузу
        simulationService.resumeTruckAI();
    } else {
        // поток на паузе → запускаем
        simulationService.pauseTruckAI();
    }
        }
    );
  ...
  // Геттеры
  public TextField getTxtCarPeriod() {
    return txtCarPeriod;
  }

  public TextField getTxtTruckPeriod() {
    return txtTruckPeriod;
  }

  public ComboBox<Integer> getCbCarProb() {
    return cbCarProb;
  }

  public ListView<Integer> getLvTruckProb() {
    return lvTruckProb;
  }

  public CheckBox getChkShowInfo() {
    return chkShowInfo;
  }

  
  public RadioButton getRbShowTime() {
    return rbShowTime;
  }

  public RadioButton getRbHideTime() {
    return rbHideTime;
  }
